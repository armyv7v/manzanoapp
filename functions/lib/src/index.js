...
function isSuperAdminEmail(raw) {
...
function assertSuperAdmin(request) {
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "Debes iniciar sesion para usar el panel Binance P2P.");
    }
    ...
    if (request.auth.token.admin !== true || !isSuperAdminEmail(email)) {
        throw new https_1.HttpsError("permission-denied", "Solo el super admin autorizado puede operar Binance P2P.");
    }
    return {
...
function sanitizeBinanceP2PActionPayload(actionType, raw) {
    if (actionType === "heartbeat") {
        return {};
    }
    const amount = typeof raw.amount === "string" ? raw.amount.trim() : "";
    if (!amount) {
        throw new https_1.HttpsError("invalid-argument", "Debes indicar un monto para preparar la venta.");
    }
...
function buildPaidOrderFinancials(orderData) {
    ...
    const sellerCommissionAmountCLP = Number(orderData.sellerCommissionAmountCLP || (useClpCommission && sellerCommissionRate > 0 && orderCLPAmount > 0
...
async function syncUserProfileDoc(params) {
    const { uid, email, isAdmin = false, isSeller = false, platform, } = params;
    const payload = {
        email: email || "",
        isAdmin,
        isSeller,
...
async function sendOrderConfirmation(apiKey, orderId, orderData) {
    ...
    const proofUrls = Array.isArray(orderData.proofUrls) ? orderData.proofUrls : (orderData.proofUrl ? [orderData.proofUrl] : []);
    const attachments = [];
...
async function syncDynamicClpBalance(reason) {
    const db = admin.firestore();
 ...
 */
exports.syncClpBalanceFromAccounts = (0, firestore_1.onDocumentWritten)("accounts/{accountId}", async (event) => {
 ...
 */
exports.syncClpBalanceFromRate = (0, firestore_1.onDocumentUpdated)("config/rate", async (event) => {
...
function collectTokensFromUser(data) {
    ...
    if (typeof webToken === "string" && webToken.trim())
        tokens.add(webToken);
...
exports.ensureUserProfile = (0, https_1.onCall)(async (request) => {
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "Debes iniciar sesion para sincronizar tu perfil.");
    }
...
exports.testPushNotification = (0, https_1.onCall)(async (request) => {
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "Must be logged in to test push.");
    }
 ...
 */
exports.requestBinanceP2PAction = (0, https_1.onCall)(async (request) => {
    ...
    if (!BINANCE_P2P_ALLOWED_ACTIONS.has(actionType)) {
        throw new https_1.HttpsError("invalid-argument", "Tipo de accion Binance P2P no soportado.");
    }
    const db = admin.firestore();
    ...
    if (!activeActionSnapshot.empty) {
        ...
        throw new https_1.HttpsError("failed-precondition", `Ya existe una accion Binance P2P activa (${activeAction.id}). Espera a que termine antes de crear otra.`);
    }
 ...
 */
exports.cancelBinanceP2PAction = (0, https_1.onCall)(async (request) => {
    assertSuperAdmin(request);
    const actionId = typeof request.data?.actionId === "string" ? request.data.actionId.trim() : "";
    if (!actionId) {
        throw new https_1.HttpsError("invalid-argument", "Debes indicar el actionId a cancelar.");
    }
    const actionRef = admin.firestore().collection("binance_p2p_actions").doc(actionId);
    const actionSnap = await actionRef.get();
    if (!actionSnap.exists) {
        throw new https_1.HttpsError("not-found", "La accion Binance P2P indicada no existe.");
    }
    const actionData = actionSnap.data() || {};
    if (actionData.status !== "pending") {
        throw new https_1.HttpsError("failed-precondition", "Solo puedes cancelar acciones Binance P2P que aun esten pendientes.");
    }
    await actionRef.set({
        status: "cancelled",
 ...
 */
exports.resendOrderEmail = (0, https_1.onCall)({ secrets: [brevoApiKey] }, async (request) => {
    if (!request.auth) {
    ...
    if (!orderId) {
        throw new https_1.HttpsError("invalid-argument", "Falta ID del pedido.");
    }
    ...
    if (orderData.status !== "Pagado") {
        throw new https_1.HttpsError("failed-precondition", "Solo se envían correos para pedidos pagados.");
    }
    if (!orderData.email) {
        throw new https_1.HttpsError("failed-precondition", "El pedido no tiene un correo válido.");
    }
    const key = brevoApiKey.value();
    if (!key) {
        throw new https_1.HttpsError("failed-precondition", "BREVO_API_KEY no configurado.");
    }
    try {
        ...
        throw new https_1.HttpsError("internal", error.message || "Error al enviar el correo.");
    }
});
/**
 * Void a paid order and reverse all accounting movements.
 */
exports.voidPaidOrder = (0, https_1.onCall)(async (request) => {
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "Debes estar autenticado.");
    }
    if (request.auth.token.admin !== true) {
        throw new https_1.HttpsError("permission-denied", "Solo los administradores pueden anular pedidos pagados.");
    }
    if (!isSuperAdminEmail(request.auth.token.email)) {
        throw new https_1.HttpsError("permission-denied", `Solo ${SUPER_ADMIN_EMAIL} puede anular pedidos pagados.`);
    }
    ...
    try {
        await db.runTransaction(async (transaction) => {
            ...
            if (orderData.status === "Cancelado") {
                throw new https_1.HttpsError("failed-precondition", "El pedido ya estaba cancelado.");
            }
            if (orderData.status !== "Pagado") {
                throw new https_1.HttpsError("failed-precondition", "Solo se pueden anular pedidos pagados desde esta operación segura.");
            }
            ...
            if (!sourceAccountId) {
                throw new https_1.HttpsError("failed-precondition", "Este pedido fue pagado sin registrar la cuenta origen. No se puede anular automaticamente.");
            }
            const accountRef = db.collection("accounts").doc(sourceAccountId);
            const rateDoc = await transaction.get(rateRef);
            const accountDoc = await transaction.get(accountRef);
            if (!accountDoc.exists) {
                throw new https_1.HttpsError("failed-precondition", "La cuenta origen registrada ya no existe.");
            }
            ...
            const pushBalanceHistory = (amount, type, note) => {
        ...
        if (error instanceof https_1.HttpsError)
            throw error;
        throw new https_1.HttpsError("internal", error?.message || "No se pudo anular el pedido pagado.");
    }
});
/**
...
async function pruneInvalidToken(token) {
    ...
    directTokenDocs.forEach((userDoc) => {
        ...
        const platformTokens = data.fcmPlatformTokens;
        ...
        Object.entries(deviceTokens).forEach(([deviceId, entry]) => {
...
async function cleanupInvalidTokens(tokens, responses) {
    ...
    responses.forEach((response, idx) => {
        if (response.success)
            return;
        const code = response.error?.code || response.error?.errorInfo?.code || "";
...
async function sendMulticastWithCleanup(message) {
 ...
 */
exports.notifyNewWholesalePurchase = (0, firestore_1.onDocumentCreated)("wholesale_purchases/{purchaseId}", async (event) => {
 ...
 */
exports.notifyOrderUpdate = (0, firestore_1.onDocumentUpdated)({ document: "orders/{orderId}", secrets: [brevoApiKey] }, async (event) => {
 ...
 */
exports.notifyBinanceP2PActionUpdate = (0, firestore_1.onDocumentUpdated)("binance_p2p_actions/{actionId}", async (event) => {
    const beforeData = event.data?.before.data();
    const afterData = event.data?.after.data();
    if (!beforeData || !afterData)
        return null;
    ...
    if (!interestingStates.has(afterData.status))
        return null;
    try {
 ...
 * =========================================================
 */
exports.binanceVpsProxy = (0, https_1.onRequest)({ cors: false }, async (req, res) => {
    ...
    try {
        ...
        if (cleanPath === '/balance')
            cleanPath = '/api/balance';
        if (cleanPath === '/p2p-rate')
            cleanPath = '/api/p2p-rate';
