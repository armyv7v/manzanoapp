(function () {
    ...
    let balanceHistoryListener = null; // Listener for VES balance history
    let clpBalanceHistoryListener = null; // Listener for CLP balance history
    let settingsListener = null; // Listener for global app settings (Store Status)
    ...
    // Alias for CLP formatting
    const formatClp = (value) => formatCurrency(value, 'es-CL', { currency: 'CLP', minimumFractionDigits: 0 });
    ...
    /**
     * Helper to simplify bank names for display (removes parenthetical details).
     * @param {string} name The full bank name.
     * @returns {string} The simplified name.
     */
    const simplifyBankName = (name) => {
        if (!name) return '';
        // Remove text inside parentheses and trim
    ...
    function formatDateForTable(date) {
        ...
        const dateStr = dateObj.toLocaleDateString('es-CL', { day: '2-digit', month: '2-digit', year: 'numeric', timeZone: 'America/Santiago' });
        const timeStr = dateObj.toLocaleTimeString('es-CL', { hour: '2-digit', minute: '2-digit', hour12: true, timeZone: 'America/Santiago' });
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // Admin Auth Modal Elements (Unified Standard)
        ...
        const clpBalanceHistoryHeader = document.getElementById('clp-balance-history-header'); // NEW: Added missing header element
        ...
        const clpBalanceHistorySummary = document.getElementById('clp-balance-history-summary'); // NEW: Added missing summary element
        ...
        const populateBankSelects = () => {
            ...
            const optionsHtml = venezuelanBanks.map(bank => `<option value="${bank}">${simplifyBankName(bank)}</option>`).join('');

            bankSelects.forEach(select => {
                const currentVal = select.value; // Preserve value if needed
        ...
        const handleClientListScroll = () => {
            ...
            requestAnimationFrame(() => {
                ...
                if (scrollTop + clientHeight >= scrollHeight - 100 && clientListPage < totalPages) {
                    ...
                    clientListPage++;
                    renderClientListPage(); // This now appends
                    // Allow next check after render
                    setTimeout(() => { isScrollHandling = false; }, 100);
                } else {
        ...
        /**
         * Renders the exchange rate ticker as an auto-rotating carousel.
         */
        ...
        const renderExchangeRateTicker = () => {
            ...
            if (availableRates.length === 0) {
                tickerWrap.innerHTML = '<div class="ticker-carousel-item active"><span>No hay tasas de cambio disponibles.</span></div>';
                return;
            }

            // Create carousel items
            const carouselItems = availableRates.map(([currency, rate], index) => {
                ...
                return `
                <div class="ticker-carousel-item ${isActive}">
                    <span class="flex items-center justify-center gap-2">
                        <span class="text-2xl">${flag}</span>
                        <span class="font-bold">1 CLP =</span>
                        <span class="text-lg">${formattedRate} ${currency}</span>
                    </span>
                </div>
            `;
            }).join('');

            tickerWrap.innerHTML = `<div class="ticker-carousel">${carouselItems}</div>`;

            // Start auto-rotation if there are multiple rates
            if (availableRates.length > 1) {
                ...
                rateCarouselInterval = setInterval(() => {
                    const items = tickerWrap.querySelectorAll('.ticker-carousel-item');
                    if (items.length === 0) return;

                    // Remove active class from current item
                    items[currentRateIndex].classList.remove('active');

                    // Move to next item
                    currentRateIndex = (currentRateIndex + 1) % items.length;

                    // Add active class to new item
                    items[currentRateIndex].classList.add('active');
        ...
        const renderAdminViewState = (state) => {
            ...
            // It should only be visible if we are in 'panel' mode
            if (adminInterface) {
                adminInterface.classList.toggle('hidden', state !== 'panel');
            }

            // Control the Modal for Login State
            if (state === 'login') {
                if (adminAuthModal) {
                    console.log('DEBUG: Opening Admin Auth Modal (Force Visible)');
                    adminAuthModal.classList.remove('hidden');
                    adminAuthModal.classList.add('flex');
                    adminAuthModal.style.display = 'flex'; // FORCE OVERRIDE
                }
            } else {
                if (adminAuthModal) {
                    adminAuthModal.classList.add('hidden');
                    adminAuthModal.classList.remove('flex');
                    adminAuthModal.style.display = 'none'; // FORCE HIDE
                }
            }

            // Control the admin panel content
            if (adminPanel) {
        ...
        // --- Layout Debugging & Sidebar Logic ---
        ...
        const toggleSidebar = () => {
            ...
            console.log(`[Sidebar Status] Visible: ${isVisible}, Classes: ${sidebar.className}`);
        };
        window.toggleSidebar = toggleSidebar; // Expose to global scope for HTML onclick

        /**
         * Sets up the listener for the store configuration (Open/Closed).
         */
        const setupStoreSettingsListener = () => {
            if (settingsListener) return; // Already listening

            settingsListener = db.collection('settings').doc('config').onSnapshot(doc => {
                if (doc.exists) {
                    const data = doc.data();
                    isStoreOpen = data.storeOpen !== false; // Default to true if undefined
                    updateStoreStatusView();
                } else {
                    // Create default config if missing
                    db.collection('settings').doc('config').set({ storeOpen: true })
                        .catch(err => console.error("Error creating default config:", err));
                }
            }, error => console.error("Error fetching store settings:", error));
        };
        ...
        const toggleStoreStatus = () => {
            ...
            db.collection('settings').doc('config').set({ storeOpen: newStatus }, { merge: true })
                .then(() => {
                    showToastNotification(newStatus ? 'Tienda Abierta' : 'Tienda Cerrada');
                })
                .catch(error => {
                    ...
                    isStoreOpen = !newStatus;
                    updateStoreStatusView();
                    showCustomAlert("Error al actualizar el estado de la tienda.");
                });
        };
        ...
        const enableLayoutDebug = () => {
            // Debug mode disabled.
        };

        // Auto-enable debug for this session if Admin
        if (window.location.search.includes('debug=true')) {
            // enableLayoutDebug();
        }

        /**
         * Switch between Main Views (User vs Admin)
         */
        const switchMainView = (view) => {
            ...
            if (isUserView) {
                ...
                // Show Main App Container (Header + Content)
                const mainAppContainer = document.getElementById('main-app-container');
                if (mainAppContainer) mainAppContainer.classList.remove('hidden');

                // FORCE SHOW User Interface
                if (userInterface) {
                    userInterface.classList.remove('hidden');
                    // Ensure the form is back in its original home for users
                    if (mainContent && !mainContent.contains(userInterface)) {
                        mainContent.prepend(userInterface);
                    }
                }
            } else { // 'admin' view requested
                // FORCE HIDE User Interface
                if (userInterface) {
                    userInterface.classList.add('hidden');
                }

                // Hide Main App Container (Header + Content)
                // This ensures the custom header and ticker are hidden in admin mode
                const mainAppContainer = document.getElementById('main-app-container');
                if (mainAppContainer) mainAppContainer.classList.add('hidden');

                // This shows the admin login form or the full panel.
                renderAdminViewState(isAdmin ? 'panel' : 'login');

                // FORCE DASHBOARD VIEW INITIALIZATION
                if (isAdmin && window.switchAdminView) {
        ...
        const switchTab = (selectedTab) => {
            tabs.forEach((tab, index) => {
                ...
                tab.classList.toggle('hover:bg-gray-200', !isSelected);
                tab.classList.toggle('hover:text-gray-700', !isSelected);

                // Note: removed old text-gray-700 and hover:bg-gray-300
            ...
            switch (order.status) {
                ...
                case 'Pagado':
                    ...
                    if (order.proofUrl) {
                        ...
                        if (isSeller) {
                            // NEW: Check if file sharing is supported natively or via Web API
      ...
      `;
            ...
            try {
                if (isNative) {
                    try {
                        ...
                        if (!Filesystem || !Share) {
                            throw new Error('Plugins de compartir no detectados en la App');
                        }

                        const fileName = `comprobante_${Date.now()}.jpg`;

                        // Usamos downloadFile de Capacitor que no sufre de errores CORS
                        ...
                        // IMPORTANTE: Convertir el path a una URI compatible con el plugin de Share
                        const uriResult = await Filesystem.getUri({
                            path: fileName,
                            directory: 'CACHE'
                        });

                        // Si llega aquí, intentamos compartir el archivo nativo usando la URI
                        await Share.share({
                    ...
                    try {
                        ...
                        const file = new File([blob], 'comprobante.jpg', { type: blob.type });
                        ...
                        if (navigator.canShare({ files: [file] })) {
                            ...
                            throw new Error('Web Share File Not Supported');
                        }
                    } catch (webError) {
                        const fallbackUrl = `https://wa.me/?text=${encodeURIComponent(shareText)}%20${encodeURIComponent(proofUrl)}`;
                        window.open(fallbackUrl, '_blank');
                ...
                showToastNotification('Error al compartir comprobante');
            } finally {
        ...
        const attachOrdersListener = () => {
            ...
            ordersListener = createdTodayQuery.onSnapshot(snapshot => {
                ...
                if (dashboardPendingCount) {
                    dashboardPendingCount.textContent = pendingOrdersCount;
                }
        ...
        const attachAccountsListener = (onFirstLoad = null) => {
            ...
            accountsListener = db.collection('accounts').onSnapshot(snapshot => {
                ...
                if (balanceOpHolderSelect) {
                    ...
                    balanceOpHolderSelect.innerHTML = `<option value="">Seleccione Titular...</option>` +
                        sortedHolders.map(holder => `<option value="${holder}">${holder}</option>`).join('');
                }

                // --- NEW: Account filtering and Recent History ---
                const updateBankSelectByHolder = (holderName) => {
                    const banks = accountsData
                        .filter(acc => acc.holder === holderName)
                        .map(acc => acc.bank);
                    const uniqueBanks = Array.from(new Set(banks)).sort();
                    balanceOpBankSelect.innerHTML = uniqueBanks.map(bank => `<option value="${bank}">${bank}</option>`).join('');
                };

                balanceOpHolderSelect.addEventListener('change', (e) => {
                    updateBankSelectByHolder(e.target.value);
                });

                const renderRecentAccounts = () => {
                    const recentAccountsContainer = document.getElementById('balance-op-recent-accounts');
                    if (!recentAccountsContainer) return;

                    const recent = JSON.parse(localStorage.getItem('recent_admin_accounts') || '[]');
                    if (recent.length === 0) {
                        recentAccountsContainer.innerHTML = '<p class="text-xs text-gray-400">Sin historial aún</p>';
                        return;
                    }

                    recentAccountsContainer.innerHTML = recent.map(acc => `
                        <button class="text-[10px] bg-blue-50 text-blue-600 px-2 py-1 rounded-full border border-blue-100 hover:bg-blue-100 transition"
                                onclick="selectRecentAccount('${acc.holder}', '${acc.bank}')">
                            ${acc.bank} (${acc.holder.split(' ')[0]})
                        </button>
                    `).join('');
                };

                window.selectRecentAccount = (holder, bank) => {
                    balanceOpHolderSelect.value = holder;
                    updateBankSelectByHolder(holder);
                    balanceOpBankSelect.value = bank;
                };

                const saveRecentAccount = (holder, bank) => {
                    let recent = JSON.parse(localStorage.getItem('recent_admin_accounts') || '[]');
                    recent = recent.filter(acc => !(acc.holder === holder && acc.bank === bank));
                    recent.unshift({ holder, bank });
                    recent = recent.slice(0, 5); // Keep last 5
                    localStorage.setItem('recent_admin_accounts', JSON.stringify(recent));
                    renderRecentAccounts();
                };

                // Inject render function into modal opening
                window.renderBalanceRecentAccounts = renderRecentAccounts;
                // --- END NEW ---
        ...
        const updateCommissionSummary = () => {
            ...
            // --- UPDATE DASHBOARD CARD ---
            const dashboardCommissionTotal = document.getElementById('dashboard-commission-total');
            if (dashboardCommissionTotal) {
                dashboardCommissionTotal.textContent = totalLine.toLocaleString('es-VE', { minimumFractionDigits: 2, style: 'currency', currency: 'VES' });
            }
      ...
      `;
        ...
        /** Attaches a real-time listener for the Admin Ticker (Exchange Rate). */
        let adminTickerListener = null;
        const attachAdminTickerListener = () => {
            if (adminTickerListener) adminTickerListener();

            adminTickerListener = db.collection('config').doc('rates').onSnapshot(doc => {
                if (doc.exists) {
                    const data = doc.data();
                    const rate = data.VES && data.VES.price ? data.VES.price : 0;

                    // Update New Ticker
                    const tickerDisplay = document.getElementById('admin-rate-display');
                    if (tickerDisplay) {
                        tickerDisplay.textContent = rate.toLocaleString('es-VE', { minimumFractionDigits: 2 });
                    }

                    // Update Dashboard Card (Legacy)
                    const cardDisplay = document.getElementById('current-rate-display');
                    if (cardDisplay) {
                        cardDisplay.textContent = rate.toLocaleString('es-VE', { minimumFractionDigits: 2 });
                    }
                }
            }, error => console.error("Error fetching admin rates:", error));
        };
        ...
        const updateUIForUser = (user) => {
            ...
            if (user) {
                ...
                user.getIdTokenResult(true).then(idTokenResult => {
                    console.log('🔍 DEBUG: Token claims loaded:', idTokenResult.claims);
                    console.log('🔍 DEBUG: Admin claim value:', idTokenResult.claims.admin);
                    console.log('🔍 DEBUG: Is admin?', !!idTokenResult.claims.admin);
                    ...
                    // --- NEW: Sync admin/seller status to Firestore users collection ---
                    // This ensures Server-side logic (Cloud Functions) knows who is an admin
                    if (user && user.uid) {
                        ...
                        }, { merge: true }).catch(err => console.error("Error syncing user status:", err));
                    }
                    ...
                    if (adminIdDisplay) {
                        const adminTag = userTags[user.email] || 'ADMIN';
                        adminIdDisplay.textContent = adminTag;
                    }
                    ...
                    if (isAdmin) {
                        ...
                        mainActionBtnContainer.classList.remove('flex');
                        switchMainView('admin');
                        adminCreateOrderToggleBtn.classList.remove('hidden'); // RESTORED: Show the button for admins
                        attachAdminTickerListener(); // Start the ticker listener for admin
                        attachAdminCommissionListener(); // NEW: Start commission listener
                        attachBankFeeListener(); // NEW: Start bank fee listener
                        setupStoreSettingsListener(); // Start listening for store open/close status
                        if (!ordersListener) {
                            // attachOrdersListener(); // Handled by app-modern.js
                            ...
                            // Attach Ticker Listener
                            attachAdminTickerListener();
                        ...
                        mainActionBtnContainer.classList.add('flex');
                        ...
                        // attachUserOrdersListener(user.uid); // Handled by app-modern.js
        ...
        authWrapper.onAuthStateChanged(updateUIForUser);
        ...
        const renderBalanceHistoryItem = (history) => {
            const date = history.timestamp ? formatDateForTable(history.timestamp.toDate()) : 'N/A';
      ...
      `;
            ...
            enableNotificationsBtn.addEventListener('click', async () => {
                ...
                try {
                    ...
                    if (permission === 'granted') {
                        ...
                        if (fcmToken) {
                            // UNIFIED: Save token directly to user document
                            // matches the logic in push-notifications.js for native app
                            ...
                            // Also keep the legacy collection for safety transition
        ...
        // Service Worker for Firebase Messaging is not needed - using native push notifications via Capacitor
        /* 
        ...
        */
        ...
        adminLoginForm.addEventListener('submit', (e) => {
            ...
            authWrapper.signInWithEmailAndPassword(email, password)
                .then(() => {
                    if (adminAuthModal) {
                        adminAuthModal.classList.add('hidden');
                        adminAuthModal.classList.remove('flex');
        ...
        adminLogoutBtn.addEventListener('click', () => {
            ...
            authWrapper.signOut(); // Para usuarios reales, onAuthStateChanged se encargará de actualizar la UI
        });

        // Admin: Update Exchange Rate
        rateForm && rateForm.addEventListener('submit', async (e) => {
            e.preventDefault();
        ...
        const openBalanceOperationModal = (type) => {
            ...
            // --- UX Improvement: Pre-select account with highest balance ---
            if (accountsData.length > 0) {
                const richest = [...accountsData].sort((a, b) => (b.balance || 0) - (a.balance || 0))[0];
                if (richest) {
                    balanceOpHolderSelect.value = richest.holder;
                    // Trigger dynamic bank filter
                    const banks = accountsData.filter(acc => acc.holder === richest.holder).map(acc => acc.bank);
                    balanceOpBankSelect.innerHTML = Array.from(new Set(banks)).sort().map(bank => `<option value="${bank}">${bank}</option>`).join('');
                    balanceOpBankSelect.value = richest.bank;
                }
            }

            // Render recent list if available
            if (window.renderBalanceRecentAccounts) window.renderBalanceRecentAccounts();
            // --- END UX ---
        };
      ...
      `;
        ...
        balanceFinalConfirmBtn.addEventListener('click', async () => {
            ...
            // Save for recent history
            if (typeof window.renderBalanceRecentAccounts === 'function') {
                // We'll call save later in the confirmed process if needed, or right now
                // Actually, let's just save it here since confirmation is usually just a click away
                const recent = JSON.parse(localStorage.getItem('recent_admin_accounts') || '[]');
                const updated = [{ holder, bank }, ...recent.filter(a => !(a.holder === holder && a.bank === bank))].slice(0, 5);
                localStorage.setItem('recent_admin_accounts', JSON.stringify(updated));
            }
      ...
      `;
        ...
        const adminFormScrollContainer = document.getElementById('admin-form-scroll-container');

        const toggleAdminOrderForm = (forceHidden = null) => {
            const adminPlaceholder = document.getElementById('admin-order-form-placeholder');
            const isCurrentlyHidden = userInterface.classList.contains('hidden');
            const shouldHide = forceHidden !== null ? forceHidden : !isCurrentlyHidden;

            if (shouldHide) {
                userInterface.classList.add('hidden');
                if (adminPlaceholder) {
                    adminPlaceholder.classList.add('hidden');
                    adminPlaceholder.classList.remove('flex');
                }
                adminCreateOrderToggleBtn.textContent = 'Ingresar Pedido';
            } else {
                userInterface.classList.remove('hidden');
                if (adminPlaceholder && adminFormScrollContainer) {
                    adminFormScrollContainer.appendChild(userInterface);
                    adminPlaceholder.classList.remove('hidden');
                    adminPlaceholder.classList.add('flex');
                }
                adminCreateOrderToggleBtn.textContent = 'Ocultar Formulario';

                // Show backdate section for admin
                const adminBackdateSection = document.getElementById('admin-backdate-section');
                if (isAdmin) adminBackdateSection.classList.remove('hidden');
            }
        };

        adminCreateOrderToggleBtn.addEventListener('click', () => toggleAdminOrderForm());
        if (closeAdminFormBtn) {
            closeAdminFormBtn.addEventListener('click', () => toggleAdminOrderForm(true));
        }

        // Optional: Close on backdrop click
        const adminPlaceholder = document.getElementById('admin-order-form-placeholder');
        if (adminPlaceholder) {
            adminPlaceholder.addEventListener('click', (e) => {
                if (e.target === adminPlaceholder) toggleAdminOrderForm(true);
            });
        }
            ...
            try {
                ...
                if (snapshot.empty) {
                    ...
                    snapshot.forEach(doc => {
                        ...
                        row.innerHTML = `
                      <td class="p-2 text-gray-600 whitespace-nowrap">${formatDateForTable(commission.timestamp.toDate())}</td>
        ...
        } // End handleSellerCommissionSearch
        ...
        async function handleClpBalanceHistorySearch(startDate, endDate) {
            ...
            // Corrected collection name if needed, and using Date objects
            const query = db.collection('clp_balance_history')
                .where('timestamp', '>=', queryStart)
                .where('timestamp', '<=', queryEnd)
                .orderBy('timestamp', 'desc');

            if (clpBalanceHistoryListener) {
                clpBalanceHistoryListener();
            }

            clpBalanceHistoryListener = query.onSnapshot(snapshot => {
                console.log("CLP History Snapshot Size:", snapshot.size, "Empty:", snapshot.empty);
                clpBalanceHistoryList.innerHTML = '';
                let totalCredit = 0;
                let totalDebit = 0;
                if (snapshot.empty) {
                    noClpBalanceHistoryMessage.classList.remove('hidden');
                    clpBalanceHistoryHeader.classList.add('hidden');
                    clpBalanceHistorySummary.textContent = '';
                } else {
                    noClpBalanceHistoryMessage.classList.add('hidden');
                    clpBalanceHistoryHeader.classList.remove('hidden');
                    ...
                    snapshot.forEach(doc => {
                        ...
                        if (isCredit) totalCredit += data.amount;
                        if (isDebit) totalDebit += data.amount;
                        ...
                        // Map database types to Credit (Abono) or Debit (Cargo)

                        if (isCredit) {
                            amountClass = 'text-green-600 font-medium';
                            credit = formatClp(data.amount);
                        } else if (isDebit) {
                            amountClass = 'text-red-600 font-medium';
                            debit = formatClp(data.amount);
                        }
                        ...
                        row.innerHTML = `
                            ...
                            <td class="p-2 text-right font-bold text-blue-600">${formatClp(data.balanceAfter || 0)}</td>
                        `;
                ...
                clpBalanceHistoryList.innerHTML = '<tr><td colspan="5" class="text-center text-red-500 p-4">Error al cargar historial CLP</td></tr>';
            });
        }
        ...
        async function fetchAndRenderBalanceHistory(startDate, endDate) {
            ...
            balanceHistoryListener = query.onSnapshot(snapshot => {
                console.log("VES History Snapshot Size:", snapshot.size, "Empty:", snapshot.empty);
                balanceHistoryList.innerHTML = '';
                if (snapshot.empty) {
                    noBalanceHistoryMessage.classList.remove('hidden');
                    ...
                    snapshot.forEach(doc => {
                        ...
                        // Map database types to Credit (Abono) or Debit (Cargo)
                        // 'admin_commission', 'fee', 'subtract', 'tillo_commission' are Money OUT (Debits from admin perspective? Or wait.
                        // Admin Commission is income for Admin.
                        // Let's re-read the batch logic.
                        // firestoreBatch.set(..., { type: 'admin_commission', ... })
                        // This record is in 'balance_history'.
                        // The User said "Historial de Movimientos (VES)".
                        // If I am Admin, and I receive commission, it should be CREDIT.
                        // But the Batch Logic Decrements the MAIN ACCOUNT.
                        // "Decrement main account balance ... -totalDebitVes".
                        // And totalDebitVes includes adminCommission.
                        // So from the perspective of the "Source Account" (which is what this history tracks), it is a DEBIT.
                        // The "Source Account" pays the commission.
                        // So yes, it is a DEBIT.
        ...
        /**
         * Gets or creates a client document in Firestore and returns its ID.
         * @param {Object} clientData - Client data from the order
         * @param {string} clientData.clientName - Client's full name
         * @param {string} clientData.cedula - Client's ID number
         * @param {string} clientData.email - Client's email
         * @param {string} [clientData.phone] - Client's phone (optional)
         * @param {string} [clientData.bank] - Client's bank (optional)
         * @param {string} [clientData.accountNumber] - Client's account number (optional)
         * @param {string} [clientData.accountType] - Client's account type (optional)
         * @returns {Promise<string>} The client document ID
         */
        const getOrCreateClient = async (clientData) => {
            ...
            if (!existingClientQuery.empty) {
                // Client exists, return its ID
                const existingClient = existingClientQuery.docs[0];
                console.log(`[getOrCreateClient] Cliente existente encontrado: ${existingClient.id}`);
                return existingClient.id;
            }

            // 2. Client doesn't exist, create new one
            const newClientData = {
                clientName,
                cedula,
                email,
            ...
            if (phone) newClientData.phone = phone;
            if (bank) newClientData.bank = bank;
            if (accountNumber) newClientData.accountNumber = accountNumber;
            if (accountType) newClientData.accountType = accountType;

            const newClientRef = await db.collection('clients').add(newClientData);
            console.log(`[getOrCreateClient] Nuevo cliente creado: ${newClientRef.id}`);
        ...
        const finalizeOrder = async (withScreenshot) => {
            ...
            try {
                // **NEW: Get or create client and add clientId to order data**
                ...
                // **NEW: Add clientId to order data**
      ...
      `;
        ...
        const openBatchSuccessModal = (proofUrls) => {
            ...
            batchSuccessOrderList.innerHTML = batchProcessData.createdOrders.map((order, index) => {
                ...
                const shareLabel = (isNativePlatform() || (navigator.share && navigator.canShare)) ? 'Compartir Archivo' : 'Compartir';
                return `
        ...
        if (batchSuccessShareAllBtn) {
            batchSuccessShareAllBtn.addEventListener('click', async () => {
                ...
                if (isNativePlatform()) {
                    await Capacitor.Plugins.Clipboard.write({ string: fullMessage });
                } else {
        ...
        console.log('DEBUG: Checking for showAdminLoginBtn...', showAdminLoginBtn);
        if (showAdminLoginBtn) {
            console.log('DEBUG: Adding click listener to showAdminLoginBtn');
            showAdminLoginBtn.addEventListener('click', (e) => {
                e.preventDefault();
                console.log('DEBUG: showAdminLoginBtn clicked! Calling switchMainView("admin")');
                switchMainView('admin');
            });
        } else {
            console.error('DEBUG: showAdminLoginBtn NOT FOUND in DOM');
        }

        // --- Admin Action Buttons Listeners (Logout & Store Toggle) ---
        const adminLogoutBtns = document.querySelectorAll('#admin-logout-btn'); // Use querySelectorAll in case of duplicates
        ...
        const adminToggleStoreBtn = document.getElementById('admin-toggle-store-btn');
        if (adminToggleStoreBtn) {
            adminToggleStoreBtn.addEventListener('click', (e) => {
                e.preventDefault();
                toggleStoreStatus();
            });
        }
        ...
        document.querySelectorAll('.admin-nav-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                if (viewName) switchAdminView(viewName);
            });
        });
        ...
        if (mobileMenuBtn && sidebar) {
            mobileMenuBtn.addEventListener('click', (e) => {
                ...
                sidebar.classList.toggle('hidden');
                sidebar.classList.toggle('flex'); // Ensure flex is toggled for display
            });
    ...
    window.switchAdminView = (viewName) => {
        ...
        document.querySelectorAll('.admin-nav-btn').forEach(btn => {
            if (btn.dataset.view === viewName) {
                ...
                btn.classList.add('bg-blue-50', 'text-blue-600', 'border-r-4', 'border-blue-600');
                btn.classList.remove('text-gray-600', 'hover:bg-gray-50', 'border-transparent');
                // Icon styling (find svg inside)
                const icon = btn.querySelector('svg');
                if (icon) icon.classList.add('text-blue-600');
            } else {
                // Inactive state styling
                btn.classList.remove('bg-blue-50', 'text-blue-600', 'border-r-4', 'border-blue-600');
        ...
        if (pageTitle) pageTitle.textContent = titleMap[viewName] || 'Admin';
